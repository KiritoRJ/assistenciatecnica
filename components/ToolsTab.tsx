...
type ToolId = 'virus-cleaner' | 'battery-diagnostics' | 'interactive-checklist' | null;

const ToolsTab: React.FC<Props> = ({ settings, currentUser, tenantId }) => {
  const [selectedTool, setSelectedTool] = useState<ToolId>(null);

  // Se o checklist interativo estiver selecionado
  if (selectedTool === 'interactive-checklist') {
    return (
      <div className="w-full max-w-7xl mx-auto space-y-4 animate-in fade-in duration-300 pb-12">
        <div className="flex items-center justify-between bg-white p-4 sm:p-5 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-1 bg-purple-50 text-purple-700 text-[10px] font-black uppercase rounded-lg border border-purple-200">
      ...
      <div className="space-y-6">
        ...
        <div className="pt-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {/* Card Checklist Interativo & Laudo Técnico */}
            <div 
              onClick={() => setSelectedTool('interactive-checklist')}
              className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden flex flex-col justify-between space-y-4 hover:border-purple-200 hover:shadow-md transition-all cursor-pointer group"
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="w-12 h-12 bg-purple-50 text-purple-600 rounded-2xl flex items-center justify-center group-hover:scale-105 group-hover:bg-purple-100 transition-all shadow-inner">
                    <ClipboardCheck size={24} />
                  </div>
                  <span className="px-2.5 py-1 bg-purple-50 text-purple-700 text-[10px] font-black uppercase rounded-lg border border-purple-200 flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-purple-500 animate-pulse"></span>
                    Disponível
                  </span>
                </div>
                <div>
                  <h3 className="text-sm font-black text-slate-800 uppercase tracking-tight group-hover:text-purple-700 transition-colors">
                    Checklist & Laudo Interativo
                  </h3>
                  <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                    Testes guiados de tela, câmeras, áudio, microfone, sensores e bateria com emissão de laudo e assinatura digital.
                  </p>
                </div>
              </div>
              <div className="pt-2 border-t border-slate-50 flex items-center justify-between text-[11px] font-bold text-slate-500 group-hover:text-purple-600 transition-colors">
                <span>Testar na Bancada</span>
                <ChevronRight size={16} />
              </div>
            </div>
