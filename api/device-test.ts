...
export default async function handler(req: any, res: any) {
  ...
  if (!cleanParam) {
    return res.status(400).json({ success: false, error: 'Token ou ID da O.S. não informado.' });
  }

  // --- GET: Buscar dados da O.S. e diagnósticos existentes ---
  if (req.method === 'GET') {
    try {
      // 1. Tenta por tracking_token primeiro
      ...
      if (!order) {
        ...
        if (fallbackError) {
          console.error('Error in fallback order query:', fallbackError);
        }
        if (fallbackOrder) {
          order = fallbackOrder;
        }
      ...
      console.error('Error in /api/device-test GET:', err);
      return res.status(500).json({ success: false, error: 'Erro ao buscar dados do teste de hardware.' });
    }
  }

  // --- POST: Salvar resultados dos testes de hardware ---
  if (req.method === 'POST') {
    ...
    try {
      ...
      console.error('Error in /api/device-test POST:', err);
