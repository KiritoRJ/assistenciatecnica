...
app.get(['/api/device-test/:idOrToken', '/api/device-test/:idOrToken/'], async (req, res) => {
  const { idOrToken } = req.params;
  const cleanParam = (idOrToken || '').trim();
  console.log(`[API DEBUG] Requisição recebida em /api/device-test/ para: "${cleanParam}"`);
  
  try {
    // Verificação de segurança: garantir que o parâmetro não seja vazio
    if (!cleanParam) {
      console.log(`[API DEBUG] Parâmetro vazio ou inválido.`);
      return res.status(400).json({ success: false, error: 'Token ou ID inválido.' });
    }
    ...
    if (!order) {
      console.log(`[API DEBUG] O.S. não encontrada no banco: "${cleanParam}"`);
...
app.post(['/api/device-test/:idOrToken', '/api/device-test/:idOrToken/'], async (req, res) => {
  const { idOrToken } = req.params;
...
async function startServer() {
  // Log global para debug em produção
  app.use((req, res, next) => {
    console.log(`[DEBUG] Requisição: ${req.method} ${req.path}`);
    next();
  });

  // API Routes (moved inside startServer for safety)
  app.get(['/api/device-test/:idOrToken', '/api/device-test/:idOrToken/'], async (req, res) => {
    const { idOrToken } = req.params;
    const cleanParam = (idOrToken || '').trim();
    console.log(`[API DEBUG] Requisição recebida em /api/device-test/ para: "${cleanParam}"`);
