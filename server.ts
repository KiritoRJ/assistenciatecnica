...
// Endpoint para envio do Laudo do Checklist Interativo
app.post('/api/checklist/submit', async (req, res) => {
  try {
    const checklistResult = req.body;
    if (!checklistResult || !checklistResult.orderId) {
      return res.status(400).json({ error: 'Dados do checklist inválidos.' });
    }

    const tenantId = checklistResult.tenantId;

    if (tenantId) {
      // 1. Salva no banco de dados cloud_data
      ...
      const list: any[] = Array.isArray(existingData?.data_json)
        ? existingData.data_json.filter((c: any) => c.orderId !== checklistResult.orderId)
        : [];
      list.unshift(checklistResult);
    ...
    // 2. Atualiza a ordem de serviço correspondente se encontrada
    if (checklistResult.orderId) {
      const { data: order } = await supabase
        .from('service_orders')
        .select('id, checklist, public_notes')
        .eq('id', checklistResult.orderId)
        .maybeSingle();

      if (order) {
        // Converte os itens aprovados para o array simples de checklist
        const passedTitles = checklistResult.tests
        ...
        await supabase
          .from('service_orders')
          .update({
            checklist: passedTitles.length > 0 ? passedTitles : order.checklist,
            public_notes: `[LAUDO DIGITAL: ${checklistResult.scorePercent}% Aprovado em ${new Date().toLocaleDateString('pt-BR')}]`
          })
          .eq('id', checklistResult.orderId);
      }
    }
    ...
    console.error('Erro ao salvar laudo de checklist:', err);
    res.status(500).json({ error: 'Erro interno ao salvar checklist' });
  }
});

// Endpoint para buscar laudo de checklist por token/OS
app.get('/api/checklist/:token', async (req, res) => {
  try {
    const { token } = req.params;
    const cleanToken = token.trim();

    // Busca ordem pelo token
    const { data: order } = await supabase
      .from('service_orders')
      .select('*')
    ...
    // Busca laudos existentes no tenant
    ...
    if (order.tenant_id) {
      ...
      if (Array.isArray(cloudRes?.data_json)) {
        checklistResult = cloudRes.data_json.find((c: any) => c.orderId === order.id) || null;
      }
    }
